...
async def botlog (String, Api, Hash):
    ...
    KanalId = await Client(CreateChannelRequest(
        title='OldUserBot BotLog',
        about=LANG['AUTO_BOTLOG'],
        megagroup=True
    ))
    KanalId = KanalId.chats[0].id

    Photo = await Client.upload_file(file='OldUserlogo.jpg')
...
if __name__ == "__main__":
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/vusalold/OldUserBot"
    config['WARN_LIMIT'] = "3"
